...
public abstract class Fish implements Drawable {
    ...
    protected final double maxAccelation; // 물고기의 가속력
    ...
    public Fish(int startX, int startY, double maxVelocity, double maxAccelation, double waterResitance) {
        message = "";

        this.maxVelocity = maxVelocity;
        this.maxAccelation = maxAccelation;
    ...
    public boolean isOutOfbound() {
        if (!(-200 < location.getX() && location.getX() < 1000) || !(-200 < location.getY() && location.getY() < 800)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isEatable(Fish fish) {
        if (fish.isPlayer()) {
            if (this.isPlayer()) {
                return this.getLevel() - fish.getLevel() > 1;
            } else {
    ...
    public void eat(Fish fish) {
        // Fish를 먹었을때의 호출되는 메소드, 화면에 그려주는건 따로 한다.
        if (this.level < 9) {
    ...
    public void move() {
        ...
        case -1:
            ...
            break;
        case 0: // moveState = Up
            accelation.setValue(0, -maxAccelation);
            break;
        case 1: // moveState = UpRight
            accelation.setValue(maxAccelation, -maxAccelation);
            imageFish.setDirection(0);
            break;
        case 2: // moveState = Right
            accelation.setValue(maxAccelation, 0);
            imageFish.setDirection(0);
            break;
